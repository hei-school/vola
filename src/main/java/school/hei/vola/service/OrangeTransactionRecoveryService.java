...
/**
 * Service responsible for recovering Orange Money transactions from the Orange API and
 * synchronizing them with our payment system.
 */
@Service
@AllArgsConstructor
@Slf4j
public class OrangeTransactionRecoveryService {
   ...
   *
   * <p>Process: 1. Fetches all transactions from Orange API for the given date 2. Persists new
   * transactions to our database 3. Triggers payment verification for any matching payments
   *
  ...
  public RecoveryResult sync(LocalDate date) {
    ...
    try {
      ...
      log.info(
          "[SYNC] Fetched {} transactions from Orange API for date {}", transactions.size(), date);

      int insertedCount = processTransactions(transactions);

      log.info(
          "[SYNC] Successfully completed sync for date {}. Inserted {} new transactions",
          date,
          insertedCount);
  ...
  private void triggerPaymentVerificationIfExists(OrangeTransaction transaction) {
    ...
    log.info(
        "[SYNC] Found payment id={} for transaction ref={}, triggering verification",
        payment.id(),
        ref);
